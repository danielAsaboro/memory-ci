...
async function main(): Promise<void> {
  ...
  if (typeof outputs.BedrockLoggingRoleArn !== "string") throw new Error("CloudFormation Bedrock logging role output is missing.");
  const loggingConfiguration = await awsJson(["bedrock", "get-model-invocation-logging-configuration"], region);
  const logging = loggingConfiguration.loggingConfig as { cloudWatchConfig?: { logGroupName?: unknown; roleArn?: unknown }; textDataDeliveryEnabled?: unknown; embeddingDataDeliveryEnabled?: unknown } | undefined;
  if (logging?.cloudWatchConfig?.logGroupName !== "/aws/bedrock/stash-production-invocations" || logging.cloudWatchConfig.roleArn !== outputs.BedrockLoggingRoleArn || logging.textDataDeliveryEnabled !== true || logging.embeddingDataDeliveryEnabled !== true) throw new Error("Independently observed Bedrock invocation logging configuration does not match the stack-owned role and log group.");
