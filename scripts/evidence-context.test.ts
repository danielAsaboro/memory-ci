...
describe("evidence context producer", () => {
  it("creates the one strict shared context only from complete live identity facts", () => {
    const context = buildEvidenceContext({ Account: "123456789012" }, {
      ...
      Parameters: [{ ParameterKey: "DatabaseSecretArn", ParameterValue: "arn:aws:secretsmanager:us-east-1:123456789012:secret:stash/database-abc" }, { ParameterKey: "BedrockModelId", ParameterValue: "anthropic.claude-3-5-sonnet-20241022-v2:0" }, { ParameterKey: "BedrockEmbeddingModelId", ParameterValue: "amazon.titan-embed-text-v2:0" }],
