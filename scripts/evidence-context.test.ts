...
describe("evidence context producer", () => {
  it("creates the one strict shared context only from complete live identity facts", () => {
    ...
    }, { COCKROACH_CLUSTER_ID: "cloud-cluster-1", COCKROACH_SQL_CLUSTER_ID: "sql-cluster-1", COCKROACH_ORGANIZATION_ID: "org-1", COCKROACH_HOST: "cluster-1.aws-us-east-1.cockroachlabs.cloud", COCKROACH_TIER: "BASIC" });
    expect(context).toMatchObject({ schemaVersion: 2, aws: { accountId: "123456789012" }, cockroach: { clusterId: "cloud-cluster-1", sqlClusterId: "sql-cluster-1", tier: "BASIC" } });
  });
