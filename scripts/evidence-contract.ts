...
const smokeReceiptSchema = evidenceContextBaseSchema.extend({
  ...
  s3: z.object({ providerRequestId: z.string().min(1), versionId: z.string().min(1), key: z.string().regex(/^artifacts\//), digest: z.string().regex(/^[a-f0-9]{64}$/), etag: z.string().min(1) }).strict(), eventBridge: z.object({ providerRequestId: z.string().min(1), eventId: z.string().min(1) }).strict(), probe: z.object({ tenantId: z.string().min(1), memoryId: z.string().min(1) }).strict(),
}).strict();
