...
describe("production evidence contract", () => {
  ...
  it("fails closed when receipts differ by account, stack, cluster, or run", () => {
    ...
    expect(() => validateCorrelatedReceipts({ smoke, vector: { ...vector, probe: { ...vector.probe, sqlClusterId: "wrong-sql-cluster" } }, ccloud }, new Date("2026-08-17T17:10:00.000Z"))).toThrow(/semantic/i);
  });
