...
export type EvaluationRun = Readonly<{
  ...
  modelId: string | null; providerRequestId: string | null;
}>;

type EvaluationRow = {
  id: string; candidate_id: string; baseline_revision: string; policy_version: string;
  status: EvaluationStatus; model_id: string | null; provider_request_id: string | null;
};
...
export class EvaluationRepository {
  ...
  async createRun(input: { id: string; candidateId: string; baselineRevision: number; policyVersion: string }): Promise<EvaluationRun> {
    const result = await this.transaction.client.query<EvaluationRow>(
      `INSERT INTO evaluation_runs
       (tenant_id, id, candidate_id, baseline_revision, policy_version, status, started_at)
       VALUES ($1,$2,$3,$4,$5,'running',now()) RETURNING *`,
      [this.transaction.tenantId, input.id, input.candidateId, input.baselineRevision, input.policyVersion],
    );
  ...
  async recordResult(input: {
    ...
    await this.transaction.client.query(
      ...
      [this.transaction.tenantId, input.id, input.runId, input.scenarioId, input.status,
