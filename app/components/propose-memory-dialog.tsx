...
type Form = { namespaceId: string; memoryClass: string; trustClass: string; canonicalText: string; sourceUri: string; sourceContent: string; signatureIdentity: string; signatureKeyId: string; signature: string };
const initial: Form = { namespaceId: "", memoryClass: "policy", trustClass: "observed", canonicalText: "", sourceUri: "", sourceContent: "", signatureIdentity: "", signatureKeyId: "", signature: "" };
...
export function ProposeMemoryDialog({ workspaceId, onClose }: { workspaceId: string; onClose: () => void }) {
    ...
    mutationFn: async () => {
      ...
      return createCandidate({
        ...
        source: { id: sourceId.current, sourceType: "operator", content: form.sourceContent, contentDigest: sourceDigest,
          sourceUri: form.sourceUri || undefined, signatureIdentity: form.signatureIdentity || undefined, signatureKeyId: form.signatureKeyId || undefined,
          ...(form.signature ? { signatureAlgorithm: "ed25519" as const, signature: form.signature } : {}) },
      }, key.current);
    },
  ...
  return <div className="modal-scrim" role="dialog" aria-modal="true" aria-labelledby="propose-memory-title"><form className="confirm-dialog" onSubmit={(event) => { event.preventDefault(); if (valid) mutation.mutate(); }}>
    ...
    <label>Trusted key ID<input aria-label="Trusted key ID" value={form.signatureKeyId} onChange={change("signatureKeyId")} /></label>
    <label>Signature<input aria-label="Source signature" value={form.signature} onChange={change("signature")} /></label>
    {form.signature ? <p role="status">Source signature supplied for server verification.</p> : null}
