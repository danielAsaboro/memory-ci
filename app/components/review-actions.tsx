...
export function ReviewActions(props: Props) {
  ...
  return <LiveReviewActions key={`${props.candidate.id}:${props.candidate.latestEvaluationId ?? "none"}:${props.candidate.latestApprovedReviewId ?? "none"}`} {...props} candidate={props.candidate} />;
}
...
function LiveReviewActions({ workspaceId, candidate, evaluation, blocked: forcedBlocked }: Props & { candidate: CandidateSummary }) {
  ...
  const evaluate = useMutation({ mutationFn: () => requestEvaluation(candidate.id, idempotencyKey(evaluateKey)), onSuccess: () => { setNotice("Evaluation queued. Waiting for evidence receipt."); void invalidate(); }, onError: (error) => setNotice(errorNotice(error, "Evaluation is unavailable.")) });
  ...
  const review = useMutation({ mutationFn: (decision: "approved" | "rejected" | "quarantined") => submitReview(candidate.id, { evaluationRunId: evaluationId ?? currentEvidence!.id, decision, reason }, idempotencyKey(reviewKey)), onSuccess: async (receipt) => { setReviewId(receipt.reviewId); setNotice(`Review ${receipt.decision}; request ${receipt.reviewId}.`); await invalidate(); }, onError: (error) => { if (error instanceof StashApiError && error.code === "stale_review") recoverStaleReview(); else setNotice(errorNotice(error, "Review is unavailable.")); } });
  const promote = useMutation({ mutationFn: () => promoteCandidate(candidate.id, { reviewId: reviewId!, stableKey, reason }, idempotencyKey(promoteKey)), onSuccess: async (receipt) => { setNotice(`Active memory revision ${receipt.revision}; version ${receipt.version}.`); await invalidate(); }, onError: (error) => { if (error instanceof StashApiError && error.code === "stale_review") recoverStaleReview(); else setNotice(errorNotice(error, "Promotion is unavailable.")); } });
  useEffect(() => {
    if (!(evaluate.isSuccess || candidate.state === "evaluating") || terminal.has(polled?.status ?? "")) return;
    let active = true; let attempt = 0; const started = Date.now(); const controller = new AbortController(); let timer: ReturnType<typeof setTimeout> | undefined;
    const schedule = (wait: number) => { const remaining = 90_000 - (Date.now() - started); if (remaining <= 0) { setNotice("Evaluation is still running. Refresh later to continue monitoring."); return; } timer = setTimeout(poll, Math.min(wait, remaining)); };
    const poll = async () => { try {
      if (Date.now() - started >= 90_000) { setNotice("Evaluation is still running. Refresh later to continue monitoring."); return; }
      if (!evaluationId) { const found = (await getEvaluations(controller.signal)).find((item) => item.candidateId === candidate.id); if (!active || controller.signal.aborted) return; if (found) { setEvaluationId(found.id); timer = setTimeout(poll, 0); return; } schedule(delays[attempt++] ?? 5_000); return; }
      const result = await getEvaluation(evaluationId, controller.signal); if (!active || controller.signal.aborted || Date.now() - started >= 90_000) return; setPolled(result);
