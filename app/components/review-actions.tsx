...
const queuedEvaluationNotice = "Evaluation queued. Waiting for evidence receipt.";
...
function LiveReviewActions({ workspaceId, candidate, evaluation, blocked: forcedBlocked }: Props & { candidate: CandidateSummary }) {
  ...
  const [polled, setPolled] = useState<EvaluationSummary | null>(null); const [evaluationId, setEvaluationId] = useState<string | null>(null); const [evaluationRequest, setEvaluationRequest] = useState<{ eventId: string } | null>(null);
  ...
  const awaitingNewEvaluation = evaluationRequest !== null && authoritativeEvaluation?.triggerEventId !== evaluationRequest.eventId;
  ...
  const displayedNotice = authoritativeRequestNotice && notice === queuedEvaluationNotice ? authoritativeRequestNotice : notice;
  ...
  }, [candidate.id, candidate.state, currentEvidenceStatus, effectiveEvaluationId, evaluate.isSuccess, evaluationRequest?.eventId, invalidate]);
  ...
  return <section className="review-actions"><div><strong>{blocked ? "Promotion blocked" : "Evaluation evidence passed"}</strong><small>{blocked ? "Approval requires passed, current, non-quarantined evidence." : "Review is bound to the completed evaluation run."}</small>{displayedNotice ? <small role="status">{displayedNotice}</small> : null}</div>
